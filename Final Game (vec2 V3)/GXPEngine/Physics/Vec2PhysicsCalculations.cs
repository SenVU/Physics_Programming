...
public static class Vec2PhysicsCalculations
{
    ...
    /// <returns>0 = begining of frame, 1 = end of frame</returns>
    public static float TimeOfImpactBall(Vec2 pNewPos1, Vec2 pOldPos1, Vec2 pPos2, float pRadius1, float pRadius2)
    {
        Vec2 velocity = pNewPos1 - pOldPos1;
        Vec2 relativePos = pOldPos1 - pPos2;
        ...
        if (a == 0) return 1; // no collision
        if (D < 0) return 1; // no collision
        if (c < 0 ) return b<0?0:1; // collision error : no collision
    ...
    /// <param name="pRadius2">The other Radius</param>
    /// <returns>Vector position of the point of impact</returns>
    public static Vec2 PointOfImpactBall(Vec2 pNewPos1, Vec2 pOldPos1, Vec2 pPos2, float pRadius1, float pRadius2)
    {
        return pOldPos1 + TimeOfImpactBall(pNewPos1, pOldPos1, pPos2, pRadius1, pRadius2) * (pNewPos1 - pOldPos1);
    }
